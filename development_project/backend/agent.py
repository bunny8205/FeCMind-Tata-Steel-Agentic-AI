...
class MaintenanceWizard:
    ...
    def _llm_plan_query(self, query: str) -> dict:
        ...
        if strict_intent and not self._explicit_plant_priority_request(query):
            ...
            plan["intent"] = "asset_diagnosis" if explicit_for_task else "general_steel"
            ...
            plan["reason"] = (
                str(plan.get("reason", "")).strip()
                + f" Corrected by verifier: {strict_intent} is a task-specific request, not a plant-priority ranking."
            ).strip()
            return plan
        ...
        if isinstance(tool_calls, list):
            tool_calls.insert(
                0,
                {
                    ...
                    "status": "success" if clean_plan["planner_status"] in {"model", "fallback", "model_label_fallback"} else "review",
                },
            )
    ...
    def _clean_llm_natural_text(self, text: str) -> str:
        ...
        if len(clean) > 1200:
            snippet = clean[:1200].rstrip()
            cut = max(snippet.rfind(". "), snippet.rfind("! "), snippet.rfind("? "))
            clean = snippet[: cut + 1].strip() if cut >= 300 else snippet.rstrip(" ,;:") + "."
        return clean
    ...
    def _build_synthesis_payload(self, result: dict, clean_plan: dict, original_answer: str) -> dict:
        ...
        return {
            ...
            "grounded_answer_excerpt": original_answer[:800],
        }
...
""".strip()
    ...
    def general_steel_report(self, query: str) -> dict:
        ...
        subject = infer_steel_subject(query)
        if self._strict_task_intent(query):
