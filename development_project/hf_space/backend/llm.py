...
class LocalLLM:
    ...
    def plan(self, query: str, context: dict | None = None) -> dict:
        """Use the configured LLM as the primary planner, with deterministic fallback only if parsing fails."""
...
""".strip()
        raw = self.generate(prompt, max_new_tokens=140, max_time=14.0)
        parsed = self._parse_plan_json(raw)
        planner_status = "model_json"
        if not parsed:
            label_prompt = f"""
...
Label:
""".strip()
            label_raw = self.generate(label_prompt, max_new_tokens=12, max_time=4.0)
            parsed = self._parse_plan_json(label_raw)
            raw = label_raw
            planner_status = "model_label"
        if not parsed:
            fallback["used_model"] = bool(raw)
            fallback["raw_model_output"] = str(raw)[:500]
            fallback["planner_status"] = "model_failed_fallback" if raw else "fallback"
        ...
        parsed["planner_status"] = planner_status
    ...
    def synthesize_final_answer(self, payload: dict, max_new_tokens: int = 900) -> str:
        """Use Qwen/the configured LLM as final-response synthesizer over locked tool facts."""
        if not self.ensure_available():
            return ""

        locked_packet = self._json_safe(payload)
        locked_text = json.dumps(locked_packet, ensure_ascii=False, indent=2)[:6500]
        prompt = f"""
...
""".strip()
        text = self.generate(prompt, max_new_tokens=max_new_tokens, max_time=42.0, min_new_tokens=0).strip()
